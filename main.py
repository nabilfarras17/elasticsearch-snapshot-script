...
def create_repo(host, bucket, role, repo_name):
    ...
    r = requests.put(url, auth=ec2_auth(), data=payload, headers={
...
def restore(host, repo_name, snapshot_name, wait_for_completion=False):
    ...
    payload = '{}'
    r = requests.post(url, auth=ec2_auth(), headers={
                      "Content-Type": "application/json"})
    return r
...
def get_repo(host):
    ...
    r = requests.get(url, auth=ec2_auth(), headers={
                     "Content-Type": "application/json"})
    return r
...
def delete_repo(host, repo_name):
    url = host + "_snapshot/" + repo_name
    r = requests.delete(url, auth=ec2_auth(), headers={
                        "Content-Type": "application/json"})
    return r
...
def post(host, json):
    url = host
    r = requests.post(url, auth=ec2_auth(), data=json, headers={
                      "Content-Type": "application/json"})
    return r
...
def put(host, json):
    url = host
    r = requests.put(url, auth=ec2_auth(), data=json, headers={
                     "Content-Type": "application/json"})
    return r
...
def delete(host, json):
    url = host
    r = requests.delete(url, auth=ec2_auth(), data=json, headers={
                        "Content-Type": "application/json"})
    return r
...
def snapshot_checker_worker(identifier, host, snapshot_name):
    while get_snapshot_status_from_list_snapshots(get_repo_detail(host, default_repo_name).json(), snapshot_name)["state"] == "IN_PROGRESS":
        print("Snapshoting on "+identifier+" "+get_snapshot_status_from_list_snapshots(
            get_repo_detail(host, default_repo_name).json(), snapshot_name)["state"])
        time.sleep(5)
        pass
    print("Snapshoting on "+host+" finished with status "+get_snapshot_status_from_list_snapshots(
        get_repo_detail(host, default_repo_name).json(), snapshot_name)["state"])
    pass
    ...
    for identifier, host in tuples.items():
        ...
        try:
            print("Create snapshot response : " +
                  create_snapshot(host, default_repo_name, snapshot_name).text)
            time.sleep(5)
...
def initiate_snapshot(tuples, s3, role):
    for identifier, host in tuples.items():
        print("Create repo response : " +
              create_repo(host, s3, role, default_repo_name).text)
        time.sleep(2)
        localtime = time.localtime()
        time_string = time.strftime("%Y-%m-%d-%H-%M-%S", localtime)
        snapshot_name = identifier+"-"+time_string
...
def initiate_restore(tuples, s3, role):
    for identifier, host in tuples.items():
        ...
        print("Get repo response : " +
              get_repo_detail(host, default_repo_name).text)
        ...
        print("Restoring response " + restore(host,
              default_repo_name, snapshot_name).text)
